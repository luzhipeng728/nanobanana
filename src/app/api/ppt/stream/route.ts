...
**16:9 幻灯片尺寸：13.33 × 7.5 英寸**

| 区域 | 范围 | 说明 |
|-----|------|-----|
| 安全区域 | x: 0.5~12.83, y: 0.5~7 | 所有内容必须在此范围内 |
| 边界检查 | x + w ≤ 12.83, y + h ≤ 7 | 元素右边/下边不能超出 |

**多卡片布局计算公式：**
\`\`\`
可用宽度 = 13.33 - 左边距 - 右边距 = 12.33 英寸
2列卡片宽度 = (12.33 - 间隙) / 2 ≈ 6 英寸
3列卡片宽度 = (12.33 - 间隙×2) / 3 ≈ 4 英寸
4列卡片宽度 = (12.33 - 间隙×3) / 4 ≈ 3 英寸
\`\`\`
...
const SLIDE_WIDTH = 13.33;  // 幻灯片宽度
const MARGIN = 0.5;         // 边距
const GAP = 0.3;            // 卡片间隙

// 2x2 网格布局
function create2x2Grid(slide, cards, startY = 1.5) {
  const cols = 2, rows = 2;
  const cardW = (SLIDE_WIDTH - MARGIN * 2 - GAP * (cols - 1)) / cols;  // = 6.115
  const cardH = 2.3;

  cards.forEach((card, i) => {
    const col = i % cols;
    const row = Math.floor(i / cols);
    const x = MARGIN + col * (cardW + GAP);  // 第1列: 0.5, 第2列: 6.915
...
\`\`\`javascript
const cols = 3;
const cardW = (SLIDE_WIDTH - MARGIN * 2 - GAP * (cols - 1)) / cols;  // = 4.01
// 同样的计算逻辑...
\`\`\`
