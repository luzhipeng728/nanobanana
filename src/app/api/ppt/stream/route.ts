...
2. **📝 内容规划**
   - 基于搜索结果，规划每页的详细内容
   - 确定哪些页面需要 AI 背景图（仅封面、过渡页）
   - 规划数据图表的真实数据（将用 addChart 代码实现）

3. **🎨 生成装饰背景图（仅限纯视觉图！）**
   ⚠️ **只生成不含文字的装饰图，数据/文字必须用代码！**
   - 为封面生成震撼的背景图（nano-banana，无文字）
   - 为章节过渡页生成氛围图（nano-banana，无文字）
   - 可选：为内容页生成小型装饰配图（不含任何标注或数据）
   - **绝对不要**生成带中文文字的信息图表！

4. **💎 制作 PPT（代码为主！）**

   **🔥 核心原则：所有数据和文字必须用代码！**
   - 📊 数据图表 → \`addChart\`（柱状图、折线图、饼图）
   - 📝 中文文字 → \`addText\`（标题、要点、说明）
   - 🔄 流程图 → \`addShape\` + \`addText\`
   - 📋 表格 → \`addTable\`
   - 🖼️ AI图片 → 仅用于背景，通过 \`slide.background = { path: url }\`

   **🚨 每页最少元素数量（强制要求）：**
   | 页面类型 | 最少元素数 | 必须包含 |
   |---------|----------|---------|
   | 封面页 | 4+ | AI背景图、addText主标题、addText副标题、装饰元素 |
   | 内容页 | 6+ | Takeaway(addText)、addChart图表、addText要点、数据来源 |
   | 数据页 | 7+ | Takeaway(addText)、addChart图表、3+个addShape数据卡片、底部要点、来源 |
   | 对比页 | 8+ | Takeaway(addText)、左右两个addShape对比区块、addText总结 |
...
**经过大量实践验证，AI 图片生成有明显局限性：**

| 内容类型 | AI 图片效果 | 推荐方案 |
|----------|------------|----------|
| 📝 中文文字 | ❌ 不可靠，经常模糊或错误 | **必须用 addText** |
| 📊 数据图表 | ❌ 数据不准确，布局不可控 | **必须用 addChart** |
| 🔄 流程图 | ❌ 文字模糊，箭头混乱 | **必须用 addShape + addText** |
| 📋 表格 | ❌ 对齐问题，数据错误 | **必须用 addTable** |
| 🖼️ 纯装饰背景 | ✅ 效果好 | 用 AI 生成（nano-banana）|
| 🎨 氛围配图 | ✅ 效果好（无文字）| 用 AI 生成（nano-banana）|

### 🚨 绝对禁止（会导致 PPT 质量差）：
1. ❌ 用 AI 生成带中文文字的信息图表作为主要内容
2. ❌ 期望 AI 图片中的数据准确无误
3. ❌ 用 AI 图片替代代码图表（addChart）
4. ❌ 依赖 AI 图片展示关键业务数据
...
\`\`\`
PPT结构：
├── 封面页：AI背景图 + 代码添加标题文字（addText）
├── 目录页：纯代码构建（addText + addShape）
├── 数据页：代码图表（addChart）+ 代码文字 + 小型AI装饰图
├── 内容页：代码布局（addShape）+ AI配图（无文字的氛围图）
└── 结尾页：AI背景图 + 代码添加文字（addText）
\`\`\`
...
| 场景 | 模型 | 说明 |
|------|------|------|
| 封面全屏背景 | \`nano-banana\` | 展厅、科技感、抽象渐变等纯视觉图 |
| 章节过渡背景 | \`nano-banana\` | 氛围图、主题相关场景图 |
| 装饰性配图 | \`nano-banana\` | 放在内容旁边的插图，**不含文字** |
| 概念示意图 | \`nano-banana\` | 产品外观、场景展示，**不含标注** |

### API 调用（仅用于装饰图）：

\`\`\`bash
# 只用 nano-banana 生成纯装饰背景图
curl -X POST "${apiBaseUrl}/api/ppt/generate-image" \\
  -H "Content-Type: application/json" \\
  -d '{"prompt": "描述纯视觉场景，不要包含文字", "model": "nano-banana", "aspectRatio": "16:9"}'
\`\`\`

**Prompt 示例（装饰背景）：**
\`\`\`
# 封面背景
"A futuristic Tesla showroom with sleek electric vehicles, dramatic lighting, purple and blue gradient atmosphere, no text, cinematic wide shot"

# 科技氛围图
"Abstract technology background with flowing data streams, gradient from purple to blue, soft glow, no text or labels"

# 产品场景图
"Tesla Cybertruck parked at a modern charging station, sunset lighting, urban environment, no text"
\`\`\`
...
**数据图表：**
\`\`\`javascript
// ✅ 正确：用 addChart 展示真实数据
slide.addChart(pptx.charts.BAR, [
  { name: '2023', labels: ['Q1', 'Q2', 'Q3', 'Q4'], values: [12, 15, 18, 22] },
  { name: '2024', labels: ['Q1', 'Q2', 'Q3', 'Q4'], values: [18, 24, 28, 35] }
], {
  x: 0.5, y: 1.5, w: 5.5, h: 3,
  showValue: true,
  chartColors: ['94A3B8', '6366F1']
});
\`\`\`

**文字内容：**
\`\`\`javascript
// ✅ 正确：用 addText 添加所有中文文字
slide.addText('营收同比增长 45%', {
...
**流程图/时间线：**
\`\`\`javascript
// ✅ 正确：用 addShape + addText 构建流程图
const steps = ['需求分析', '方案设计', '开发实施', '上线运营'];
steps.forEach((step, i) => {
  // 圆形节点
  slide.addShape(pptx.ShapeType.ellipse, {
    x: 0.5 + i * 2.3, y: 2, w: 0.8, h: 0.8,
    fill: { color: '6366F1' }
  });
  // 步骤文字
  slide.addText(step, {
    x: 0.3 + i * 2.3, y: 2.9, w: 1.2, h: 0.4,
    fontSize: 10, color: '1E293B', align: 'center'
  });
  // 连接线
  if (i < steps.length - 1) {
    slide.addShape(pptx.ShapeType.line, {
      x: 1.3 + i * 2.3, y: 2.4, w: 1.5, h: 0,
...
### 第三步：生成装饰背景图（仅 2-3 张纯视觉图！）
⚠️ **只用 nano-banana 生成不含文字的背景图：**
- 🖼️ 封面：主题相关的震撼背景图（无文字！）
- 🎨 过渡页：章节氛围图（无文字！）
- 🏁 结尾：总结性视觉背景（无文字！）

**❌ 绝对不要生成：** 带中文的信息图表、数据图表、流程图！

### 第四步：编写 PPT 脚本（代码为主！）
使用 Skill 工具调用 pptx 技能，编写 create-ppt.js：

**🔥 核心原则（必须遵守）：**
1. 所有数据图表 → 用 \`addChart\`（柱状图、折线图、饼图）
2. 所有中文文字 → 用 \`addText\`（标题、要点、数据标签）
3. 流程图/时间线 → 用 \`addShape\` + \`addText\` 组合
4. AI 图片 → 只用于 \`slide.background = { path: url }\`

**代码结构：**
\`\`\`javascript
// 封面：AI背景 + 代码文字
slide1.background = { path: coverImageUrl };
slide1.addText('标题', { ... });

// 数据页：纯代码
slide2.addChart(pptx.charts.BAR, realData, { ... });
slide2.addText('数据来源：...', { ... });
\`\`\`
...
### 第六步：验证
确认 ${outputPath} 已生成，检查文件大小是否合理（通常 1-5MB）。

---
**立即开始执行！记住：代码为主 + AI背景为辅。数据图表、文字内容必须用 PptxGenJS 代码，AI 图片只用于装饰背景！**`;
