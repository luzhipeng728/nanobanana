...
5. **research_topic**: 🔥 深度研究工具（自动多轮搜索直到信息充足）
...
### 🔥 何时使用 research_topic（深度研究）vs web_search（快速搜索）

#### research_topic - 需要全面信息时使用
...
**research_topic 调用示例：**
用户需求：「苏州7日游，显示每天天气」
→ 调用 research_topic(topic="苏州未来7天天气预报", required_info=["每日天气", "温度范围", "降水概率"])

工具会自动进行多轮搜索：
1. 第一轮：搜索基础天气信息
2. 评估充足度：如果温度信息不足 → 继续搜索
3. 第二轮：补充搜索温度和穿衣建议
4. 直到信息充足或达到最大轮次
